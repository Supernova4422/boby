...
func TestParse(t *testing.T) {
	...
	test_service_subject.AddMessage(test_sender, test_msg_sent) // Message to repeat

	// Get messages and evaluate
	test_service_subject.Run()
	result_message, result_sender := test_service_sender.PopMessage()
	if result_sender != test_sender {
...
func TestEmpty(t *testing.T) {
	...
	// Message to repeat.
	test_sender := service.User{Name: "Test_User", Id: test_service_subject.Id()}
	test_service_subject.AddMessage(test_sender, "Test1")            // Message to repeat
	test_service_subject.AddMessage(test_sender, test_cmd+"Message") // Message to repeat
